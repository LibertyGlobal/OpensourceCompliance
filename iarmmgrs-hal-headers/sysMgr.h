...
typedef enum _SYSMgr_EventId_t {
	...
	IARM_BUS_SYSMGR_EVENT_CONNECTION_LOST,		/*!< Lost Connection Detection */
...
typedef struct _IARM_Bus_SYSMgr_GetEISSSavedData_Param_t
{
    unsigned int filterStatus;
    unsigned int appIDStatus;
    unsigned char appIDList[4][6];
    unsigned int appIDCount;
} IARM_Bus_SYSMgr_GetEISSSavedData_Param_t;
...
/*
 * Declare RPC API names for EISS data
 */
#define IARM_BUS_SYSMGR_API_GetEISSSavedData	"GetEISSSavedData" /*!< Gets the EISS saved data*/
...
#define IARM_BUS_SYSMGR_ConnectionLost_MaxLen 1024
...
/*! Parameter for connection lost call*/
typedef struct _IARM_Bus_SYSMgr_ConnectionLostData_t{
    char connectionLostData[ IARM_BUS_SYSMGR_ConnectionLost_MaxLen + 1 ];   //[in]  intrusion detection data
} IARM_Bus_SYSMgr_ConnectionLostData_t;
